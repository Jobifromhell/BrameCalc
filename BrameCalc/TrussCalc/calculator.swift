...
struct TrussCalculator {
    ...
    static func adjustDimensions(length: Double, width: Double?, height: Double? = nil, trussList: [TrussElement]) -> (Double, Double?, Double?) {
        ...
        let adjustedHeight = height.map { adjustDimension($0) }
    ...
    static func calculateGrill(length: Double, width: Double, trussList: [TrussElement], selectedTruss: inout [TrussElement], trussCount: inout [String: Int], totalWeight: inout Double, totalVolume: inout Double, totalPins: inout Int, volumeMargin: Double = 0.1) {
        let availableTrusses = availableTrusses(from: trussList)
        selectedTruss = []
        trussCount = [:]
        totalWeight = 0.0
        ...
        // Adjust dimensions to the nearest possible values based on available truss lengths
        let (adjustedLength, adjustedWidth, _) = adjustDimensions(length: length, width: width, trussList: availableTrusses)
        
        // Deduct 0.5 meter for length and width to account for corners (each corner 0.5m)
                let effectiveLength = adjustedLength - 1.0
                let effectiveWidth = adjustedWidth! - 1.0
                
                // Calculate horizontal trusses (length and width)
                calculateTrussesForDimension(dimension: effectiveLength, quantity: 2, trussList: availableTrusses, selectedTruss: &selectedTruss, trussCount: &trussCount, totalPins: &totalPins)
                calculateTrussesForDimension(dimension: effectiveWidth, quantity: 2, trussList: availableTrusses, selectedTruss: &selectedTruss, trussCount: &trussCount, totalPins: &totalPins)
        
        // Add corners
        if let corner = availableTrusses.first(where: { $0.type == .corner4Way }) {
            addTrussToCount(truss: corner, quantity: 4, trussCount: &trussCount, selectedTruss: &selectedTruss, totalPins: &totalPins)
        }
        // Calculate total weight and volume
        for (key, quantity) in trussCount {
